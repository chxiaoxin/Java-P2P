...
*Run this program before runnin TCPActivePeer.java program 
*Accept one command input:folder path
...
class TCPPassivePeer {
    ...
    {   /**
        *start two threads in main fucntion
        */
        com.path=argv[0];
     ...
     static class sendFileName implements Runnable{
        ...
        */
        ...
        public void run(){
            ...
            try{
                com.p2pSocket_list=new Socket(com.remoteIP,6789);
                for (File file:files) {
                    if (file.isFile()) {
    ...
    static class getFileName implements Runnable{
        ...
        */
        ...
        public void run(){
            try{
                com.welcomeSocket_list = new ServerSocket(6001);
            }catch(IOException e){e.printStackTrace();}
                File[] files = new File(com.path).listFiles();
                for (File file:files) {
                    if (file.isFile()) {
                ...
                while(true) {
                    try{
                        ...
                        InetSocketAddress sockaddr = (InetSocketAddress)com.connectionSocket_list.getRemoteSocketAddress();
                        InetAddress inaddr = sockaddr.getAddress();
                        Inet4Address in4addr = (Inet4Address)inaddr;
                        byte[] ip4bytes = in4addr.getAddress(); 
                        String ip4string = in4addr.toString();
                        com.remoteIP=ip4string.substring(1,ip4string.length());
    ...
    static class sendFile implements Runnable{
        ...
        */
        ...
        public void run(){
            try{
            com.p2pSocket_file=new Socket(com.remoteIP,7002);
            }catch(IOException e){e.printStackTrace();}
            try{
    ...
    static class getFile implements Runnable{    
        /**
        *getFile thread:get the file through file transfer socket and write them into given *folder according to path command parameter,filename can be obtained from t_fList after *that, start sendFileName thread and sendFile thread in the end of this thread. 
        */
        private Thread t_gf;
        getFile(){}
        public void run(){
            try{
                com.welcomeSocket_file=new ServerSocket(6002);
            }catch(IOException e){e.printStackTrace();}
            while(true){
                try{  
